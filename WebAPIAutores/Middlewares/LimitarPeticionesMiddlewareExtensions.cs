...
namespace WebAPIAutores.Middlewares
{
    ...
    public class LimitarPeticionesMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, ApplicationDbContext context)
        {
            ...
            var llaveDB = await context.LlavesAPI.Include(x => x.RestriccionesDominio).Include(x => x.RestriccionesIP).Include(x => x.Usuario).FirstOrDefaultAsync(x => x.Llave == llave);
            if(llaveDB == null)
            {
                httpContext.Response.StatusCode = 400;
            ...
            if(llaveDB.TipoLlave == TipoLlave.Gratuita)
            {
                ...
                if (cantidadPeticionesRealizadasHoy >= limitarPeticionesConfiguracion.PeticionesPorDiaGratuito)
                {
                    ...
                    await httpContext.Response.WriteAsync("Ha excedido el limite de peticiones por día. Si desea realizar mas peticiones actualice su suscripcion a una cuenta profesional.");
                    return;
                }
            ...
            var superRestricciones = PeticionSuperaAlgunaDeLasRestricciones(llaveDB, httpContext);

            if(!superRestricciones)
            {
        ...
        {
            var hayRestricciones = llaveAPI.RestriccionesDominio.Any() || llaveAPI.RestriccionesDominio.Any();
            ...
            var peticionSuperLasRestriccionesDeIP = PeticionSuperaLasRestriccionesDeIP(llaveAPI.RestriccionesIP, httpContext);


            return peticionSuperaLasRestriccionesDeDominio || peticionSuperLasRestriccionesDeIP;
        }
