...
namespace WebAPIAutores.Controllers
{
    ...
    public class FacturasController : ControllerBase
    {
        ...
        public async Task<ActionResult> Pagar(PagarFacturaDTO pagarFacturaDTO)
        {
            ...
            //Se simula que el pago fue exitoso
            facturaDB.Pagada = true;
            await context.SaveChangesAsync();

            var hayFacturasPendientesVencidas = await context.Facturas.AnyAsync(x => x.UsuarioId == facturaDB.UsuarioId && !x.Pagada && x.FechaLimiteDePago < DateTime.Today);
